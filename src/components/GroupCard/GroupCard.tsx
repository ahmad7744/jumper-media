                    ...
                    <div className="flex items-center gap-5">
                        ...
                        {priorityLabel && (
                            <div className="bg-zinc-800 px-2 py-[6px] rounded-full items-center">
                                <p className="text-xs text-zinc-300 font-semibold">#{priorityLabel}</p>
                            </div>
                        )}
                    </div>
                    <div className="cursor-pointer" dangerouslySetInnerHTML={{ __html: Assets.DotIcon }} />
                </div>
                <p className="text-zinc-300 text-xs font-medium mt-2">All Phones: {totalDevices}</p>
                
                <div className="flex items-center justify-between">
                    <div className="flex flex-col gap-1 mt-6">
                        <div className="text-green-400 bg-transparent" dangerouslySetInnerHTML={{ __html: Assets.OnlineIcon }} />
                        <p className="text-green-400 text-xs font-medium">Active Phones</p>
                        <p className="text-zinc-200 text-[22px] font-bold">{activeDevices} <span className="text-sm font-normal">({activePercentage}%)</span></p>
                    </div>
                    <div className="flex flex-col gap-1 mt-6">
                        <div className="text-red-400 bg-transparent" dangerouslySetInnerHTML={{ __html: Assets.OfflineIcon }} />
                        <p className="text-red-400 text-xs font-medium">Inactive Phones</p>
                        <p className="text-zinc-200 text-[22px] font-bold">{inactiveDevices} <span className="text-sm font-normal">({inactivePercentage}%)</span></p>
                    </div>
                </div>
            </div>
